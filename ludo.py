...
def aggresive_move(my_player, dice_value):
    ...
    for i in range(4):
        ...
        if (my_loc == -2 or my_loc == -1 or Board[my_loc].home_lane_flag == True):
            continue
        for j in range(4):
            correct_ind = invalidHomeLane(my_player.ptokenlist[i],
                                          dice_value)  # returns the correct index of mytoken if required
            # checks if the other's token is not in safe state
            if ((Board[correct_ind].safe_state_flag == False) and (
                correct_ind == opp_player.ptokenlist[j].getlocation())):
                return my_player.ptokenlist[i]
    return None
...
def getFastMovePiece(my_player, dice_value):
    ...
    for i in range(4):
        ...
        if (my_player.ptokenlist[i].counter + dice_value > 57):
            continue
        if (l != -1 and l != -2 and (my_player.ptokenlist[i].color != Board[my_player.ptokenlist[
            i].getlocation()].color)):  # do this only if the piece is out or not and not in home lane
            if valid_move(my_player.ptokenlist[i], dice_value):
            ...
            if REPEAT == False:
                ...
                if 'SIXES' in dice:
                        ...
                        if (my_player.out_pieces == 0):  # all the 4 tokens are inside home state
                            ...
                            if (dice_value == 6 or dice_value == 1) and (my_player.out_pieces != 4):
                                ...
                                if (t == None):
                                    ...
                                    my_player.ptokenlist[t.id].setlocation(
                                        ind)  # set the token to start position+dicevalue
                    ...
                    sys.stderr.write("my token pos are>>>" + str(my_player.ptokenlist[0].getlocation()) + '>>>' + str(
                        ...
                        my_player.ptokenlist[2].getlocation()) + '>>>' + str(
                        my_player.ptokenlist[3].getlocation()) + '\n')
